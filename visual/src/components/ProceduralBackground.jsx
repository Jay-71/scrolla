...
function neonColor() {
    const neons = [
        'hsl(280, 100%, 70%)',  // electric purple
        'hsl(320, 100%, 65%)',  // hot pink
        'hsl(180, 100%, 60%)',  // cyan
        'hsl(140, 100%, 55%)',  // neon green
        'hsl(40,  100%, 60%)',  // amber
        'hsl(200, 100%, 65%)',  // electric blue
        'hsl(340, 100%, 60%)',  // magenta
        'hsl(60,  100%, 60%)',  // neon yellow
        'hsl(260, 100%, 75%)',  // violet
        'hsl(160, 100%, 55%)',  // mint neon
    ];
...
function NeonOrbits() {
    const circles = useMemo(() =>
        Array.from({ length: Math.floor(randRange(5, 10)) }, () => ({
            ...
            delay: rand() * -15,
            opacity: randRange(0.7, 1.0),   // was 0.4–0.8
            glowR: randRange(8, 16),
        }))
        , []);
        ...
        <svg viewBox="0 0 100 100" className="proc-svg">
            <defs>
                {circles.map((c, i) => (
                    <filter key={`f${i}`} id={`glow${i}`}>
                        <feGaussianBlur stdDeviation={c.glowR / 3} result="blur" />
                        <feMerge>
                            <feMergeNode in="blur" />
            ...
            {circles.map((c, i) => (
                <circle key={i} r={c.r} fill={c.color} opacity={c.opacity} filter={`url(#glow${i})`}>
                    ...
                    />
                    <animate attributeName="r" values={`${c.r};${c.r * 1.6};${c.r}`} dur="2s" repeatCount="indefinite" />
                </circle>
            ))}
        </svg>
    );
...
function ElectricGrid() {
    const lines = useMemo(() => {
        const arr = [];
        for (let i = 0; i < 8; i++) {
            arr.push({ x: 12.5 * i, color: neonColor(), duration: randRange(1.5, 4), delay: rand() * 3, opacity: randRange(0.5, 0.85) });
            arr.push({ y: 12.5 * i, color: neonColor(), duration: randRange(1.5, 4), delay: rand() * 3, opacity: randRange(0.5, 0.85) });
        }
        return arr;
    }, []);
        ...
        <svg viewBox="0 0 100 100" className="proc-svg">
            {lines.map((l, i) =>
                l.x !== undefined ? (
                    <line key={i} x1={l.x} y1="0" x2={l.x} y2="100"
                        stroke={l.color} strokeWidth="0.5" opacity={l.opacity}>
                        <animate attributeName="opacity" values={`${l.opacity};${Math.min(l.opacity * 2, 1)};${l.opacity}`}
            ...
            {pulses.map((p, i) => (
                <circle key={`p${i}`} cx={p.x} cy={p.y} r={p.r} fill={p.color} opacity="0">
                    <animate attributeName="opacity" values="0;1;0" dur={`${p.duration}s`}
                        repeatCount="indefinite" begin={`${p.delay}s`} />
                    <animate attributeName="r" values={`${p.r};${p.r * 5};${p.r}`} dur={`${p.duration}s`}
                        repeatCount="indefinite" begin={`${p.delay}s`} />
                </circle>
            ))}
        </svg>
...
function MeteorShower() {
    const meteors = useMemo(() =>
        Array.from({ length: Math.floor(randRange(8, 15)) }, () => {
            ...
            return {
                x1: startX, y1: startY,
                x2: startX + Math.cos(angle * Math.PI / 180) * len,
                y2: startY + Math.sin(angle * Math.PI / 180) * len,
                color: neonColor(), duration: randRange(1, 3),
                delay: rand() * 8, width: randRange(0.5, 1.5),  // was 0.3–1
                opacity: randRange(0.7, 1.0),                   // was 0.4–0.9
            };
        })
        , []);
        ...
        <svg viewBox="0 0 100 100" className="proc-svg">
            ...
            {meteors.map((m, i) => (
                <line key={i} x1={m.x1} y1={m.y1} x2={m.x2} y2={m.y2}
                    stroke={`url(#mg${i})`} strokeWidth={m.width} strokeLinecap="round" opacity="0">
...
function BurstRings() {
    const rings = useMemo(() =>
        Array.from({ length: Math.floor(randRange(6, 10)) }, () => ({
            cx: randRange(20, 80), cy: randRange(20, 80),
            maxR: randRange(10, 30), color: neonColor(),
            duration: randRange(2, 5), delay: rand() * 6,
            strokeWidth: randRange(0.6, 1.5),   // was 0.3–1
        }))
        , []);
...
function PlasmaBlobs() {
    const blobs = useMemo(() =>
        Array.from({ length: Math.floor(randRange(6, 11)) }, () => ({
            cx: randRange(10, 90), cy: randRange(10, 90),
            r: randRange(8, 22), color: neonColor(),    // bigger blobs
            dx: randRange(-20, 20), dy: randRange(-20, 20),
            duration: randRange(4, 10), delay: rand() * -8,
            opacity: randRange(0.5, 0.75),              // was 0.2–0.5
        }))
        , []);
